<template>
  <div class="mapLayout">
    ...
    <!-- <div class="mapstyle_class">
            <el-select v-model="mapstyle" style="width:100px;" @change="changeType">
...
const emits = defineEmits(["changeCenter"]);
...
const changeType = () => {
  var styleName = "amap://styles/" + mapstyle.value;
...
const initAMap = () => {
  ...
  AMapLoader.load({
    ...
    .then((AMap) => {
      ...
      var opts = { subdistrict: 0, extensions: "all", level: "city" };
      aMap.value = AMap;
      //利用行政区查询获取边界构建mask路径
      //也可以直接通过经纬度构建mask路径
      ...
      //添加高度面
      // var object3Dlayer = new aMap.value.Object3DLayer({ zIndex: 1 });
      // map.value.add(object3Dlayer)
      // var height = -8000;
      // var color = '#0088ffcc';//rgba
      // var wall = new aMap.value.Object3D.Wall({
      //     path: bounds,
      //     height: height,
      //     color: color
      // });
...
// 标记点
const setPoint = () => {
  var icon = new aMap.value.Icon({
    size: new aMap.value.Size(26, 40), // 图标尺寸
    image: util.getAssetsFile("icon.png"), // Icon的图像
    imageSize: new aMap.value.Size(26, 40), // 根据所设置的大小拉伸或压缩图片
  });
  ...
  pointData.value.forEach((item) => {
    ...
    marker.on("click", (res) => {
      ElMessage.success(res.target._opts.extData.name);
...
.amap-marker-label {
  background: url("@/assets/mapTitle.png");
